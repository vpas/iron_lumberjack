...
//	public Texture[] textures;
	...
	float lastTimeCheckedIsMoving = 0;

	// Update is called once per frame
	void FixedUpdate () {
		if (Time.time - lastLogTime > 5) {
//			DebugConsole.Log ("Character netId: " + netId + " coords: " + transform.position + " mat: " + characterMaterial.GetInstanceID() + " health: " + health + "char: " + charName);
		...
		if (Time.time - lastTimeCheckedIsMoving < 1) {
			isMoving = wasMoving;
		} else {
			lastTimeCheckedIsMoving = Time.time;
			if (!inAttackingState) {
